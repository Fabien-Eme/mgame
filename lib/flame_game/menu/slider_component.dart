...
class SliderComponent extends PositionComponent with HasGameReference {
  ...
  FutureOr<void> onLoad() {
    ...
    addAll([
      ...
      ...listSliderMiddle,
      sliderEnd,
      selector = SelectorCursor(onDrag: (double x) => changeSelectorPosition(x)),
    ]);
...
class SelectorCursor extends SpriteComponent with HasGameReference, DragCallbacks {
  void Function(double) onDrag;
  SelectorCursor({required this.onDrag, super.position});
  ...
  void onDragUpdate(DragUpdateEvent event) {
    onDrag(event.canvasDelta.x * 1.5);
